			...
			if (event.type == EventType::LightStateChanged && !event.data.empty()) {
				try {
					...
					if (json.contains("on") && json["on"].contains("on")) {
						std::cout << "    Power: "
							<< (json["on"]["on"].get<bool>() ? "ON" : "OFF")
							<< std::endl;
					}

					if (json.contains("dimming") && json["dimming"].contains("brightness")) {
						std::cout << "    Brightness: "
							<< json["dimming"]["brightness"].get<double>()
							<< "%" << std::endl;
					}
					...
					if (json.contains("color")) {
						...
						if (color.contains("xy") &&
							color["xy"].contains("x") &&
							color["xy"].contains("y")) {

							std::cout << "    Color XY: ("
								<< color["xy"]["x"].get<double>() << ", "
								<< color["xy"]["y"].get<double>() << ")"
								<< std::endl;
						}
					}

					if (json.contains("color_temperature")) {
						const auto& colorTemperature = json["color_temperature"];
						if (colorTemperature.contains("mirek") &&
							colorTemperature.contains("mirek_valid") && colorTemperature["mirek_valid"] == true) {

							std::cout << "    Color Temperature: "
								<< colorTemperature["mirek"].get<int>()
								<< " mirek" << std::endl;
						}
					}
				}
				...
				std::cout << "    Raw event: " << json.dump(4) << std::endl;
